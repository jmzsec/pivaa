...
pipeline {
    ...
    stages {
		...
		stage ('MobSF') {
			steps {
				sh '''curl -F \ $TESTE http://localhost:8000/api/v1/upload -H "X-Mobsf-Api-Key:8176dec5ce2b50f71eb7759b43439b69eac8bdca572b686d7741abf0c14ff239" | awk -F\'[/"]\' \'{print $8}\' > hash.txt'''
